...
assert(_destroyed);
_monitor.lock();
SYS_LOG(DEBUG,"_workItem.size: %d,_listSize: %d,_procSize: %d", _workItems.size(),_listSize,_procSize);
...
if ( _destroyed)
{
//SYS_LOG(ERROR,"%s","ThreadPoolDestroyedException");
return -1;
}
if (_listSize > _listSizeMax )
{
//SYS_LOG(DEBUG,"%s","ThreadPoolQueueFullException");
return -1;
}
