...
namespace util
{
util::Service* util::Service::_instance=NULL;
static util::CtrlCHandler* _ctrlCHandler=NULL;

static void ctrlCHandlerCallback( int sig )
{
util::Service* service = util::Service::instance();
assert( service != 0 );
...
try
{
_ctrlCHandler = new util::CtrlCHandler;
...
SOCKET fds[2];
const int iRet = utilInternal::createPipe(fds);
if ( iRet != 0 )
{
