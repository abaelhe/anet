...
SYS_LOG(ERROR, "创建目录失败: %s", m_queuePath);
return;
}
...
openWriteFile();
if (m_writeFd == -1) {
SYS_LOG(WARN, "文件没打开: %s:%u", m_queuePath, m_head.write_seqno);
return EXIT_FAILURE;
}
}
if (data == NULL || len == 0) {
...
openReadFile();
if (m_readFd == -1) {
SYS_LOG(WARN, "文件没打开: %s:%d", m_queuePath, m_head.read_seqno);
return NULL;
}
}
int ret, size = 0;
...
if (retSize < 0) {
SYS_LOG(ERROR, "读出错, m_readFd:%d, %s(%d)", m_readFd, strerror(errno), errno);
break;
}
...
SYS_LOG(WARN, "读出的size不正确:%d,curPos:%d,readOff:%d,retry:%d,seqno:%u,m_readFd:%d",
...
SYS_LOG(WARN, "读文件不正确:%d<>%d,curPos:%lld,readOff:%d,retry:%d,seqno:%u,m_readFd:%d",
...
// 重新读
int64_t curPos = lseek(m_readFd, 0-ret-retSize, SEEK_CUR);
SYS_LOG(WARN, "flag不正确:item->flag(%d)<>FLAG(%d),curPos:%lld,readOff:%d,retry:%d,seqno:%u,m_readFd:%d",
...
// 重新读
int64_t curPos = lseek(m_readFd, 0-ret-retSize, SEEK_CUR);
SYS_LOG(WARN, "读出的len不正确:%d<>%d,curPos:%lld,readOff:%d,retry:%d,seqno:%u,m_readFd:%d",
...
if (retSize > 0) {
SYS_LOG(WARN, "IO忙,retSize:%d,seqno:%u", retSize, m_head.read_seqno);
...
SYS_LOG(INFO, "打开下一文件:%s,wseq:%u,rseq:%u,roffset:%d,m_readFd:%d",
...
SYS_LOG(ERROR, "写失败: %s", m_queuePath);
}
}
return EXIT_SUCCESS;
}
