...
sys::CThreadMutex Socket::_dnsMutex;
...
SYS_LOG(DEBUG, "打开, fd=%d, addr=%s", _socketHandle, getAddr().c_str());
return (0 == ::connect(_socketHandle, (struct sockaddr *)&_address, sizeof(_address)));
}

void Socket::close() {
if (_socketHandle != -1) {
SYS_LOG(DEBUG, "close fd=%d, addr=%s, err:%s", _socketHandle, getAddr().c_str(), strerror(errno));
::close(_socketHandle);
_socketHandle = -1;
}
}
...
if (res > 0) {
//SYS_LOG(INFO, "写出数据, fd=%d, addr=%d", _socketHandle, res);
...
if (res > 0) {
//SYS_LOG(INFO, "读入数据, fd=%d, addr=%d", _socketHandle, res);
...
return sys::CNetUtil::ipToAddr(peer.sin_addr.s_addr, ntohs(peer.sin_port));
}
return 0;
}
